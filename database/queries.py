...
def _execute_sync(query: str, params: tuple) -> int: # FIXED: return type is now int
    ...
    try:
        ...
        rowcount = cursor.rowcount # FIXED: get row count
        conn.commit()
        return rowcount # FIXED: return row count
...
def _query_sync(query: str, params: tuple, as_dict: bool = False) -> Optional[List[Union[Tuple, Dict]]]: # CHANGED
    ...
    try:
        ...
        if as_dict: # NEW: Логика для возврата словарей
...
async def db_execute(query: str, params: tuple = ()) -> int: # FIXED: return type is now int
...
async def db_query(query: str, params: tuple = (), as_dict: bool = False) -> Optional[List[Union[Tuple, Dict]]]: # CHANGED
    """Асинхронно выполняет SELECT и возвращает все строки."""
    loop = asyncio.get_running_loop()
    func = partial(_query_sync, query, params, as_dict) # CHANGED
    return await loop.run_in_executor(None, func)
